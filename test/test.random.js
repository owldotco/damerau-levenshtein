...
describe('natural.DamerauLevenshteinDistance', () => {
  it(
    'should generate distances',
    () => {
      const t0 = Date.now();
      for (const [src, target] of testStringPairs) {
        distances.push(
          natural.DamerauLevenshteinDistance(src, target, {
            search: true,
          }).distance
        );
      }
...
describe('n-api DamerauLevenshteinDistance', () => {
  const nApiDistances = [];
  ...
  it('should generate correct distances', () => {
    ...
    for (const [src, target] of testStringPairs) {
      nApiDistances.push(
        native.DamerauLevenshteinDistance(src, target, { search: true })
      );
    }
